Highcharts.getJSON('https://cdn.jsdelivr.net/gh/highcharts/highcharts@v7.0.0/samples/data/world-population-density.json', function (data) {
    ...
    var alpha = [{ "id": 4, "name": "Afghanistan", "alpha2": "af", "alpha3": "afg" },
    ...
    function reName(str) { //str mene gelen ad
        let a;
        alpha.forEach(element => {
            if (str.toLowerCase() == element.alpha2) {

                str = element.alpha3.toUpperCase();
    ...
    for (i = 0; i < map.countries.length; i++) {
        var innerMap = {};
        innerMap["code3"] = reName(map.countries[i].name);

        innerMap["value"] = map.countries[i].count;
        processed_json.push(innerMap);

    }
    processed_json.forEach(element => {
        console.log(element)
    });
    // Initiate the chart
